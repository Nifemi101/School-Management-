...
export async function submitPasswordResetRequest({
  ...
  if (existingRequest) {
    return {
      success: false,
      error: 'A reset request is already pending for this ID. Please contact your admin.',
    }
  }
  ...
  if (role === 'student') {
    ...
    const nameMatch =
      student.first_name.toLowerCase().trim() === first_name.toLowerCase().trim() &&
    ...
    const nameMatch =
      teacher.first_name.toLowerCase().trim() === first_name.toLowerCase().trim() &&
  ...
  if (!profile) {
    return { success: false, error: 'Account not found. Please contact your admin.' }
  }
...
export async function setNewPassword(newPassword: string) {
  ...
  // Confirm this is actually a temp login session
  const { data: profile } = await supabaseAdmin
    .from('profiles')

...
import {
  ...
  CheckCircle, GraduationCap, Info, AlertCircle,
} from 'lucide-react'
...
export default function StudentSubjectsPage() {
  ...
  const handleEnroll = async (subjectId: string) => {
    ...
    if (error) {
      ...
      setEnrollError(`Could not enroll in this subject. Please try again.`)
    } else if (data) {
  ...
  const handleUnenroll = async (subjectId: string) => {
    ...
    if (error) {
      ...
      setEnrollError(`Could not remove this subject. Please try again.`)
    } else {
  ...
  const getClassName = (subject: Subject) => {
    ...
    return (subject.classes as { name: string }).name ?? 'All Classes'
  }
